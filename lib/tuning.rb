def three_stage_guess_pars(page,xheight,meta_threshold:0.5,verbosity:1)
  ...
  # The squirrel algorithm also has a parameter k, which is not set here. It defaults to 3, but when threshold3 is low, it goes lower.
  # The code to do this is in match.rb.
  threshold1,threshold2,threshold3,laxness = [0.2,0.4,0.85,0.0]
  tighten = meta_threshold-0.5
  if tighten<0 then
    a1,a2,a3,al = [0.8,1.8,1.2,-2.0]
    x = 10**(-2*tighten)
    if x>30 then x=30 end
  else
    a1,a2,a3,al = [0.8,0.6,0.17,0.0]
    x = 1
  end
