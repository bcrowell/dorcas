class Job
  ...
  def init_helper(data,key,default)
    ...
    if key=='spacing_multiple' then @spacing_multiple = value end
    if key=='threshold' then @threshold = value end
    if key=='cluster_threshold' then @cluster_threshold = value end
    if key=='adjust_size' then @adjust_size = value end
  end
