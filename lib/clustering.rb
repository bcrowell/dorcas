def find_clusters(c,threshold,used:[])
  ...
  # If items have a correlation lower than threshold, then we assume they're not in the same category, i.e., the cluster is limited in radius.
  ...
  0.upto(n-1) { |i|
    ...
    results.each { |already_a_member|
      if c[already_a_member][i]<threshold then not_like_us=true; break end
    }
    if not_like_us then next end
    results.push(i)
  }
  others = find_clusters(c,threshold,used:used.union(results))
